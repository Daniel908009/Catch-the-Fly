...
# function for loading all the backgrounds from the backgrounds folder
def load_backgrounds():
    backgrounds = []
    for background in os.listdir('backgrounds'):
        # checking if the file is an image
        if background.endswith('.png') or background.endswith('.jpg') or background.endswith('.jpeg'):
            backgrounds.append(pygame.transform.scale(pygame.image.load(f'backgrounds/{background}'), (width, height)))
    # if there are no backgrounds, then creating a default one
    if len(backgrounds) == 0:
        backgrounds.append(pygame.Surface((width, height)))
        backgrounds[0].fill((0, 0, 0))
    return backgrounds
...
def resize_screen():
    ...
    settings_img = pygame.transform.scale(pygame.image.load('assets/settings.png'), (base_size, base_size))
    speed = base_size // 12
    fly_img = pygame.transform.scale(pygame.image.load('assets/fly.png'), (base_size, base_size))
...
number_of_flies = 1
font = pygame.font.Font('freesansbold.ttf', base_size // 2)
slap_sound = mixer.Sound('assets/slap.mp3')
sounds = True
music = True
